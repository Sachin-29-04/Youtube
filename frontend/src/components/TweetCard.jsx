...
} from "../services/tweet/tweetApi";
import { formatTimeAgo } from "../utils/formatTimeAgo";
import toast from "react-hot-toast";

const TweetCard = memo(({ data, addTweet = false }) => {
  const { data: tweetsData, refetch } = useGetUserTweetQuery(data, {
    skip: !data,
  });
        ...
        <form onSubmit={handleTweetSubmit} className="w-full">
          <div className="mt-2 border pb-2">
            ...
            <div className="flex items-center justify-end gap-x-3 px-3">
              <button
                type="submit"
                className="bg-[#08e6f5] px-3 py-2 font-semibold text-black"
              >
                Send
              </button>
            </div>
        ...
        {tweets.length > 0 ? (
          [...tweets]?.reverse().map((tweet, idx) => (
            ...
            >
              ...
              <button
                className="sm:mr-5 flex items-start"
                onClick={(e) => {
                  e.preventDefault();
